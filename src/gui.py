...
class AutomatonGUI:
    ...
    def setup_ui(self):
        ...
        # String validation controls
        validate_frame = ttk.LabelFrame(main_frame, text="Validar cadena")
        validate_frame.pack(fill='x', pady=(0, 10))
        ttk.Label(validate_frame, text="Cadena a validar:").pack(side='left', padx=5)
        self.string_entry = ttk.Entry(validate_frame, width=40)
        self.string_entry.pack(side='left', padx=5)
        ttk.Button(validate_frame, text="Validar", command=self.validate_string_gui).pack(side='left', padx=5)
        self.validation_result = ttk.Label(validate_frame, text="")
        self.validation_result.pack(side='left', padx=10)
    ...
    def validate_string_gui(self):
        input_str = self.string_entry.get()
        automaton = self.minimized_dfa or self.dfa or self.nfa
        
        if not automaton:
            self.validation_result.config(text="No hay autómata cargado", foreground="orange")
            return
        if not input_str:
            self.validation_result.config(text="Ingrese una cadena", foreground="orange")
            return
        try:
            accepted = automaton.validate_string(input_str)
            if accepted:
                self.validation_result.config(text="Cadena aceptada", foreground="green")
            else:
                self.validation_result.config(text="Cadena rechazada", foreground="red")
        except Exception as e:
            self.validation_result.config(text=f"Error: {str(e)}", foreground="red")
    ...
    def save_file(self, file_format):
        ...
        if not file_path:
            return
        try:
            write_automaton(result_automaton, file_path, 'json')
