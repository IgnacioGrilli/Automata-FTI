...
def build_arg_parser():
    ...
    p.add_argument(
        "input", nargs="?", help="Archivo de entrada (.json) con un AFN (ε permitido)"
    )
    p.add_argument(
        "-o",
        "--output",
        help="Archivo de salida (.json). Por defecto, junto a la entrada",
    )
    p.add_argument(
        "--in-format",
        choices=["json"],
        help="Forzar formato de entrada (auto por extensión)",
    )
    p.add_argument(
        "--out-format",
        choices=["json"],
        help="Forzar formato de salida (auto por extensión)",
    )
    p.add_argument(
        "--no-minimize", action="store_true", help="No minimizar (emitir AFD crudo)"
    )
